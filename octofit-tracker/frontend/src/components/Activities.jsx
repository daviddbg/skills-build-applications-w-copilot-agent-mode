      ...
      {!state.loading && !state.error && (
        <ul className="list-group">
          {state.items.map((item) => (
            <li className="list-group-item" key={item._id ?? item.id ?? `${item.type}-${item.loggedAt}`}>
              <strong>{item.type ?? 'Unknown'}</strong> <span className="text-muted">{item.durationMinutes ?? 0} min</span>
            </li>
          ))}
        </ul>
      )}
