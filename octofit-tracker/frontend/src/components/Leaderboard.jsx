      ...
      {!state.loading && !state.error && (
        <ul className="list-group">
          {state.items.map((item) => (
            <li className="list-group-item" key={item._id ?? item.id ?? `${item.displayName}-${item.rank}`}>
              <strong>#{item.rank ?? '-'}</strong> {item.displayName ?? 'Unknown'} <span className="text-muted">{item.points ?? 0} pts</span>
            </li>
          ))}
        </ul>
      )}
